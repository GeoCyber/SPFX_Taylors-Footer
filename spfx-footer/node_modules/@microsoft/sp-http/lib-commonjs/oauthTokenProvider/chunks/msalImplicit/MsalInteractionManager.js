    ...
    * @returns A promise that resolves when you may safely interact with the MSAL.js client.
    ...
    MsalInteractionManager.acquireControl = function () {
        return new Promise(function (resolve, reject) {
            if (!MsalInteractionManager.isRequestInProgress) {
                MsalInteractionManager.isRequestInProgress = true;
                resolve();
            }
            else {
                MsalInteractionManager.requestQueue.push(resolve);
                setTimeout(function () {
                    var isCurrentRequest = resolve === MsalInteractionManager.requestQueue[0];
                    if (isCurrentRequest) {
                        MsalInteractionManager.isRequestInProgress = false;
                    }
                    resolve();
                }, MsalInteractionManager.DEFAULT_AUTO_RESOLVE_TIME);
            }
        });
    };
    /**
    ...
    MsalInteractionManager.releaseControl = function () {
        MsalInteractionManager.isRequestInProgress = false;
        var nextTask = MsalInteractionManager.requestQueue.shift();
        if (nextTask) {
            nextTask();
        }
    };
    MsalInteractionManager.isRequestInProgress = false;
    MsalInteractionManager.requestQueue = [];
    // We'll wait a maximum of 10 seconds before automatically closing
    MsalInteractionManager.DEFAULT_AUTO_RESOLVE_TIME = 10000;
