...
class CustomizeWebpackConfigurationPlugin {
    ...
    apply(heftSession, heftConfiguration, options) {
        if (!options.customizationFilename) {
            throw new Error('A customization filename must be provided.');
        }
        heftSession.hooks.build.tap("spfx-webpack-configuration-plugin" /* SpfxWebpackConfigurationPlugin */, (build) => {
            build.hooks.bundle.tap("spfx-webpack-configuration-plugin" /* SpfxWebpackConfigurationPlugin */, (bundleSubstage) => {
                bundleSubstage.hooks.configureWebpack.tapPromise({
                    ...
                    await this._updateWebpackConfigurationAsync(scopedLogger, options.customizationFilename, heftConfiguration, webpackConfiguration, build.properties);
                    return webpackConfiguration;
                });
            });
        });
    }
    async _updateWebpackConfigurationAsync(scopedLogger, customizationFilename, heftConfiguration, webpackConfiguration, buildStageProperties) {
        const customizeConfigFilePath = path.join(heftConfiguration.projectConfigFolder, customizationFilename);
